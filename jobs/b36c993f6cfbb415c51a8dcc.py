...
class AetherLabScene(Scene):
    def construct(self):
        ...
        # Note: coil_color_tracker is already used by the coil updater above;
        # driving it from `ignite` would conflict with the manual set_value calls
        # in ACT 3 (which would be overwritten). Skip the extra updater layer
        # and instead animate it directly during ignition.
        ...
        # Cache the y-axis top so we don't depend on NumberLine.y_range (which
        # does not exist on NumberLine in current Manim Community).
        y_top = ax.y_range[1]

        # Always_redraw with Mobjects that wrap the tracker value, so the
        # returned object is a Mobject (Line/Text) and not a bare numpy scalar.
        wien_tick = always_redraw(lambda: Line(
            ax.c2p(peak_wl_tracker.get_value(), 0),
            ax.c2p(peak_wl_tracker.get_value(), y_top),
        ...
        wien_label = always_redraw(lambda: Text(
            ...
            ax.c2p(peak_wl_tracker.get_value(), y_top * 0.92), UP, buff=0.08))
        ...
        # Compose the complete Planck law as a single typeset line using a fraction structure
        frac = MathTex(
            r"B_\lambda(T) \;=\;",
        ...
        # Build the live curve as a VGroup-wrapped mobject so the surrounding
        # always_redraw returns a real Mobject (not a bare scalar). We use a
        # stable mobject that refreshes itself through a single-updater pattern:
        live_curve = ax_in.plot(
            lambda nm: planck_nm_to_y(nm, T_now) * 1.0,
            x_range=[200, 3000, 8], color=PRIMARY, stroke_width=2.5,
        )
        # Move live_curve on top of the static curve_in (which stays as a
        # faded reference) so that the breathing animation is visible.
        live_curve.set_stroke(opacity=0.95)
