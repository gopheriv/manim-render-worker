...
class AetherLabScene(Scene):
    def construct(self):
        ...
        self.play(t_tracker.animate.set_value(3050), t_readout.animate.set_value(3050), run_time=0.4)
        self.wait(0.5)
        ...
        peak_lam = b_Wien * 1e6 / t_tracker.get_value()
        peak_y = min(planck_curve(peak_lam, t_tracker.get_value()), 5)
        peak_dot = Dot(color=ACCENT, radius=0.1, point=axes.c2p(peak_lam, peak_y))
        guide_line = DashedLine(
            start=axes.c2p(peak_lam, peak_y),
            end=axes.c2p(peak_lam, 0),
            color=ACCENT,
            stroke_width=2,
        )
        peak_val = DecimalNumber(peak_lam, num_decimal_places=2, color=ACCENT).scale(0.45)
        peak_val.move_to(axes.c2p(peak_lam, 0) + UP * 0.25)
        peak_label = Text("λ_peak =", color=ACCENT, font_size=22).next_to(peak_val, LEFT, buff=0.1)
        ...
        self.play(
            ...
            t_readout.animate.set_opacity(1.0).set_value(4500),
        ...
        new_peak_lam = b_Wien * 1e6 / 4500
        new_peak_y = min(planck_curve(new_peak_lam, 4500), 5)
        peak_dot2 = Dot(color=ACCENT, radius=0.1, point=new_axes.c2p(new_peak_lam, new_peak_y))
        ...
        c_const = Text("c = 2.998e8 m/s", color=GRAY, font_size=18).next_to(h_const, DOWN, buff=0.05).align_to(h_const, RIGHT)
        kb_const = Text("k_B = 1.381e-23 J/K", color=GRAY, font_size=18).next_to(c_const, DOWN, buff=0.05).align_to(h_const, RIGHT)
        ...
        self.play(t_tracker.animate.set_value(6000), t_readout.animate.set_value(6000), Create(planck_6000), FadeIn(label_4500), FadeIn(label_6000), run_time=0.9)
        self.wait(0.6)
        ...
        self.play(filament.animate.set_opacity(0.85), run_time=0.4)
        self.play(filament.animate.set_opacity(1.0), peak_dot.animate.scale(1.25), run_time=0.4)
        self.play(peak_dot.animate.scale(1 / 1.25), run_time=0.2)
        self.wait(2.0)