...
class AetherLabScene(Scene):
    def construct(self):
        ...
        # Create spiral path for dominoes - ensure they fit within frame
        spiral_path = VGroup()
        spiral_points = []
        for i in range(20):
            angle = i * PI / 4
        ...
        # First domino falls in spiral position, revealing 1/0! = 1
        domino1 = Rectangle(width=1.0, height=0.5, color=BLUE, fill_opacity=0.7, stroke_width=2)
        domino1_label = MathTex(r"\frac{1}{0!} = 1", color=WHITE).scale(0.35)
        domino1_label.move_to(domino1.get_center())
        domino1_group = VGroup(domino1, domino1_label)
        domino1_group.move_to(spiral_points[0])
        
        self.play(FadeIn(domino1_group), run_time=0.8)
        # Domino falls effect - rotate and settle
        self.play(Rotate(domino1_group, angle=-PI/2, about_point=domino1_group.get_bottom()),
                  domino1_group.animate.move_to(spiral_points[0]), run_time=1.0)

        # Second domino shows 1/1! = 1 in spiral position
        domino2 = Rectangle(width=1.0, height=0.5, color=BLUE, fill_opacity=0.7, stroke_width=2)
        domino2_label = MathTex(r"\frac{1}{1!} = 1", color=WHITE).scale(0.35)
        domino2_label.move_to(domino2.get_center())
        domino2_group = VGroup(domino2, domino2_label)
        domino2_group.move_to(spiral_points[1])
        
        self.play(FadeIn(domino2_group), run_time=0.8)
        self.play(Rotate(domino2_group, angle=-PI/2, about_point=domino2_group.get_bottom()),
                  domino2_group.animate.move_to(spiral_points[1]), run_time=1.0)

        # Third domino shows 1/2! = 0.5 in spiral position
        domino3 = Rectangle(width=1.0, height=0.5, color=BLUE, fill_opacity=0.7, stroke_width=2)
        domino3_label = MathTex(r"\frac{1}{2!} = 0.5", color=WHITE).scale(0.35)
        domino3_label.move_to(domino3.get_center())
        domino3_group = VGroup(domino3, domino3_label)
        domino3_group.move_to(spiral_points[2])
        
        self.play(FadeIn(domino3_group), run_time=0.8)
        self.play(Rotate(domino3_group, angle=-PI/2, about_point=domino3_group.get_bottom()),
                  domino3_group.animate.move_to(spiral_points[2]), run_time=1.0)
        ...
        # EVOLVE — Dominoes accelerate as terms get smaller but accumulate faster in spiral
        ...
        for i, term in enumerate(domino_terms):
            domino = Rectangle(width=1.0, height=0.5, color=BLUE, fill_opacity=0.7, stroke_width=2)
            domino_lab = MathTex(term, color=WHITE).scale(0.3)
            domino_lab.move_to(domino.get_center())
            domino_grp = VGroup(domino, domino_lab)
            
            domino_grp.move_to(spiral_points[i+3])
                
            domino_groups.append(domino_grp)
            self.play(FadeIn(domino_grp), run_time=0.6)
            self.play(Rotate(domino_grp, angle=-PI/2, about_point=domino_grp.get_bottom()),
                      domino_grp.animate.move_to(spiral_points[i+3]), run_time=0.8)
        ...
        final_domino = Rectangle(width=1.0, height=0.5, color=RED, fill_opacity=0.9, stroke_width=2)
        final_label = MathTex(r"\sum_{n=0}^{\infty}\frac{1}{n!}", color=WHITE).scale(0.35)
        final_label.move_to(final_domino.get_center())
        final_domino_group = VGroup(final_domino, final_label)
        final_domino_group.move_to(spiral_points[7])
        
        self.play(FadeIn(final_domino_group), run_time=0.5)
        # Attempt to rotate but stop mid-way to show resistance
        self.play(Rotate(final_domino_group, angle=-PI/4, about_point=final_domino_group.get_bottom()),
                  final_domino_group.animate.move_to(spiral_points[7]), run_time=1.0)
        ...
        # All dominoes reverse direction in impossible backward cascade with spiral motion
        all_dominoes = [domino1_group, domino2_group, domino3_group] + domino_groups
        for i, domino in enumerate(reversed(all_dominoes)):
            # Reverse the spiral motion
            spiral_pos = spiral_points[len(spiral_points)-len(all_dominoes)+i-1]
            self.play(Rotate(domino, angle=PI/2, about_point=domino.get_bottom()),
                      domino.animate.move_to(spiral_pos), run_time=0.3)
