...
class AetherLabScene(Scene):
    def construct(self):
        ...
        angle_arc2 = Arc(radius=0.5, start_angle=-PI/2, angle=0, color=BLUE_E)
        
        # Labels
        theta1_label = MathTex(r"\theta_1", color=BLUE_E).scale(0.6).next_to(angle_arc1, LEFT, buff=0.1)
        theta2_label = MathTex(r"\theta_2", color=BLUE_E).scale(0.6).next_to(angle_arc2, LEFT, buff=0.1)
        
        l1_label = MathTex(r"L_1 = 1.0", color=GOLD_E).scale(0.5).next_to(rod1, RIGHT, buff=0.1)
        l2_label = MathTex(r"L_2 = 1.0", color=GOLD_E).scale(0.5).next_to(rod2, RIGHT, buff=0.1)
        
        # Position vectors
        r1_vector = Arrow(pivot, bob1.get_center(), color=YELLOW, buff=0, stroke_width=3, max_stroke_width_to_length_ratio=5)
        r2_vector = Arrow(pivot, bob2.get_center(), color=GREEN, buff=0, stroke_width=3, max_stroke_width_to_length_ratio=5)
        
        r1_label = MathTex(r"\vec{r}_1", color=YELLOW).scale(0.5).next_to(r1_vector.get_center(), LEFT)
        r2_label = MathTex(r"\vec{r}_2", color=GREEN).scale(0.5).next_to(r2_vector.get_center(), LEFT)
        ...
        def update_angle_arc2(arc):
            ...
            new_arc = Arc(radius=0.5, start_angle=-PI/2 + theta1, angle=theta2, color=BLUE_E)
        ...
        def update_r1_label(lbl):
            lbl.next_to(r1_vector.get_center(), LEFT)
        
        def update_r2_label(lbl):
            lbl.next_to(r2_vector.get_center(), LEFT)
        
        def update_theta1_label(lbl):
            lbl.next_to(angle_arc1, UR if get_theta1(t_tracker.get_value()) >= 0 else DR, buff=0.1)
        
        def update_theta2_label(lbl):
            pos1 = get_bob1_position(t_tracker.get_value())
            lbl.next_to(angle_arc2, UR if get_theta2(t_tracker.get_value()) >= 0 else DR, buff=0.1)
        ...
        def get_theta1_chaos(t):
            return 0.8 * np.sin(1.5 * t) + 0.3 * np.sin(2.3 * t)
        
        def get_theta2_chaos(t):
            return 0.6 * np.sin(2.1 * t + PI/3) + 0.4 * np.sin(1.8 * t)
        ...
        def update_angle_arc2_chaos(arc):
            ...
            new_arc = Arc(radius=0.5, start_angle=-PI/2 + theta1, angle=theta2, color=RED_E)
            new_arc.move_arc_center_to(pos1)
            arc.become(new_arc)
        ...
        golden_arc2 = Arc(radius=0.5, start_angle=-PI/2 + PI/4, angle=PI/4, color=GOLD_E)
        golden_arc2.move_arc_center_to(final_pos1)
        
        golden_theta1 = MathTex(r"\theta_1 = 45^\circ", color=GOLD_E).scale(0.6).next_to(golden_arc1, UR, buff=0.1)
        golden_theta2 = MathTex(r"\theta_2 = 45^\circ", color=GOLD_E).scale(0.6).next_to(golden_arc2, UR, buff=0.1)
        ...
        # IDLE LOOP - Subtle harmonic oscillation between the two masses
        t_idle = ValueTracker(0)
        
        def idle_update_rod1(rod):
            current_t = t_idle.get_value()
            theta = final_theta1 + 0.1 * np.sin(2 * current_t)
        ...
        def idle_update_rod2(rod):
            current_t = t_idle.get_value()
            theta = final_theta1 + 0.1 * np.sin(2 * current_t)
            pos1 = pivot + L1 * 1.8 * np.array([np.sin(theta), -np.cos(theta), 0])
            theta2 = final_theta2 + 0.1 * np.sin(2.5 * current_t)
