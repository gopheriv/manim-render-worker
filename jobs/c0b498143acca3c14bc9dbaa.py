...
class AetherLabScene(Scene):
    def construct(self):
        ...
        aperture = always_redraw(lambda: Circle(
            radius=0.5,
            color=ManimColor(interpolate_color(
                color_to_rgb("#5A1100"),
                color_to_rgb("#FFB070"),
                glow_tracker.get_value())),
            fill_opacity=0.65 + 0.35 * glow_tracker.get_value(),
            stroke_opacity=0.0,
        ).move_to(aperture_rim.get_center()))

        kicker = Text("Blackbody Furnace", color=ACCENT,
                      font_size=30).next_to(furnace_body, DOWN, buff=0.25)
        ...
        x_label = Text("wavelength (μm)", color="#9AA7B8",
                       font_size=24).next_to(axes.x_axis, RIGHT, buff=0.15)
        y_label = Text("spectral radiance", color="#9AA7B8",
                       font_size=24).next_to(axes.y_axis, UP, buff=0.1)

        # Rayleigh-Jeans ~ 2*c*k*T / lambda^4, scaled for visualization.
        T = 5800.0
        rj = axes.plot(
            lambda x: min(11.5, 0.35 / (x ** 2)),
            x_range=[0.32, 3.6], color=SECONDARY, stroke_width=4,
        )
        classical_label = Text("Classical (Rayleigh–Jeans)",
                               color=SECONDARY, font_size=30).next_to(
        ...
        readout = always_redraw(lambda: VGroup(
            Text("T = ", color=ACCENT, font_size=30),
            DecimalNumber(temp_tracker.get_value(), num_decimal_places=0,
                          color=ACCENT, font_size=30),
            Text(" K", color=ACCENT, font_size=30),
        ).arrange(RIGHT, buff=0.08).next_to(axes, UP, buff=0.2)
            .align_to(axes, RIGHT))

        uv_marker = always_redraw(lambda: Dot(
            axes.c2p(0.35, 0.35 / (0.35 ** 2)),
            color=PRIMARY, radius=0.09))

        catastrophe = Text("UV catastrophe", color=PRIMARY,
                           font_size=30).next_to(axes, DOWN, buff=0.25)
        ...
        self.play(temp_tracker.animate.set_value(6400.0),
                  uv_marker.animate.move_to(axes.c2p(0.32, 11.5)),
        ...
        # B ~ x^{-5} / (exp(C2/(T*x)) - 1)
        def planck(x):
            C2 = 1.4388  # m·K  (scaled constant for visual)
            x = np.asarray(x)
        ...
        planck_label = Text("Planck (measured)", color=PRIMARY,
                            font_size=30).next_to(axes, UP, buff=0.2) \
            .align_to(axes, LEFT)
        ...
        hero_label.next_to(planck_label, RIGHT, buff=0.6) \
            .align_to(planck_label, UP)
        ...
        def breathing_color():
            t = breath.get_value()
            return ManimColor(interpolate_color(
                color_to_rgb("#5A1100"),
                color_to_rgb("#FFB070"),
