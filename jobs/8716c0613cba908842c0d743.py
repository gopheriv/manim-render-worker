...
class AetherLabScene(Scene):
    def construct(self):
        ...
        trajectory_curve = VMobject(color=GOLD, stroke_width=4)
        trajectory_curve.set_points_as_corners([trajectory(0), trajectory(0)])
        
        def update_trajectory(traj):
            t = t_tracker.get_value()
            new_points = [trajectory(i * time_flight / 100) for i in range(int(t * 100 / time_flight) + 1)]
            if len(new_points) > 1:
                traj.set_points_as_corners(new_points)
            elif len(new_points) == 1:
                traj.set_points_as_corners([new_points[0], new_points[0]])
            return traj
        ...
        # Animate the ball along the trajectory with proper tracing
        trajectory_curve.add_updater(update_trajectory)
        ...
        def glow_animation(mob):
            scale_factor = 1 + 0.05 * np.sin(self.time * 3)  # Small pulsing effect
            original_scale = mob.width / 0.4  # Store original size
            new_width = 0.4 * scale_factor
            mob.stretch_to_fit_width(new_width)
            mob.stretch_to_fit_width(0.4)  # Reset width but keep animation
            return mob
        
        trajectory_curve.add_updater(pulse_animation)
        moving_ball.add_updater(glow_animation)
        
        self.wait(4.0)  # Idle loop duration
        trajectory_curve.remove_updater(update_trajectory)  # Clean up updater when done