...
class AetherLabScene(Scene):
    def construct(self):
        ...
        # Idle loop - pendulum continues oscillating, equations rotate
        pendulum.add_updater(update_pendulum)
        
        def rotate_equations(mob):
            mob.rotate(0.005)
        
        series.add_updater(rotate_equations)
        assumption.add_updater(rotate_equations)
        contradiction.add_updater(rotate_equations)
        
        self.play(
            t.animate.set_value(20.0),
            run_time=6.0,
            rate_func=linear
        )
        ...
        # Clean up updaters
        pendulum.remove_updater(update_pendulum)
        series.remove_updater(rotate_equations)
        assumption.remove_updater(rotate_equations)
        contradiction.remove_updater(rotate_equations)