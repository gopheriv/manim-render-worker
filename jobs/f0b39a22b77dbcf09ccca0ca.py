...
def normalize_rj(lam_nm):
    vals = np.array([rayleigh_jeans(l, T_SUN) for l in lam_nm])
    return vals / vals.max()


class AetherLabScene(Scene):
    def construct(self):
        # --- Background scaffolding (subtle) ---
        ...
        # Anchor composition regions (anchor_cell grid)
        # Filament zone: B3 (center-left)
        # Graph zone: D4 (right)
        # Hero equation: top-right

        # --- Filament disc (the physical protagonist) ---
        ...
        rad_tracker = ValueTracker(0.0)
        rad_at_300_tracker = ValueTracker(0.0)
        rad_at_500_tracker = ValueTracker(0.0)

        temp_label = Text("T =", color="#FFD580").scale(0.32).next_to(filament, DOWN, buff=0.2).align_to(filament, LEFT)
        temp_num = DecimalNumber(0, num_decimal_places=0, color="#FFD580").scale(0.34).next_to(temp_label, RIGHT, buff=0.1)
        ...
        # Color updater for filament (dim red -> yellow-white)
        ...
        filament.add_updater(lambda m: m.set_stroke(get_filament_color(), width=3))
        filament_glow.add_updater(lambda m: m.set_fill(get_filament_color(), opacity=0.18 + 0.5 * (temp_tracker.get_value() / T_SUN)))

        temp_num.add_updater(lambda m: m.set_value(temp_tracker.get_value()))
        ...
        # Pre-compute curve points (static spectra, not in always_redraw)
        lam_pts = np.linspace(200, 1200, 220)
        planck_vals = normalize_planck(lam_pts)
        rj_vals_raw = np.array([rayleigh_jeans(l, T_SUN) for l in lam_pts])
        # Clip Rayleigh-Jeans at a visible scale (it diverges) — scale so it matches Planck at long λ
        # Use a compression that shows divergence but stays inside frame
        rj_clip = np.clip(rj_vals_raw, 0.0, 4.0 * planck_vals.max())
        rj_norm = rj_clip / planck_vals.max()
        # Where it would overflow, hold near top
        rj_norm = np.clip(rj_norm, 0.0, 1.15)
        ...
        # Live readout markers on the curves
        ...
        # Energy steps beneath gold curve
        ...
        for cx, w in zip(step_x_centers, step_widths):
            # height grows as wavelength shrinks (smaller cx -> taller)
            h = 0.08 + 0.22 * (1.0 - (cx - 200) / 1000.0)
            rect = Rectangle(width=w * (axes.x_axis.get_length() / 1000.0),
                             height=h * (axes.y_axis.get_length() / 1.05),
                             color="#FFD580", fill_opacity=0.35, stroke_width=1)
            # position bottom of step at y=0, center x at cx
        ...
        panel_1_txt_2 = Text("diverges", color="#5BA8FF").scale(0.26)
        panel_1_txt_2.next_to(panel_1_txt_1, DOWN, buff=0.08)
        ...
        panel_2_txt_2 = Text("finite peak", color="#FFD580").scale(0.26)
        panel_2_txt_2.next_to(panel_2_txt_1, DOWN, buff=0.08)
        ...
        # Idle-loop pulse target on gold curve
        idle_pulse_tracker = ValueTracker(0.0)
        live_planck_curve = always_redraw(lambda: planck_curve.copy().set_stroke(opacity=1.0))
        ...
        # Temperature climb to 5800 K + glow brighten
        self.play(
            temp_tracker.animate.set_value(T_SUN),
            rad_tracker.animate.set_value(1.0),
            run_time=0.818,
            rate_func=linear,
        )
        self.wait(0.273)
        ...
        # ====================================================================
        # Spectral graph appears; empty stage
        self.play(
            Create(axes),
            FadeIn(x_label),
            FadeIn(y_label),
        ...
        rad_at_300_num.add_updater(lambda m: m.set_value(planck_spectral_radiance(300, T_SUN) / planck_spectral_radiance(500, T_SUN)))
        self.wait(1.168)
        ...
        # ====================================================================
        # Blue Rayleigh-Jeans curve draws in
        rad_at_300_num.set_value(0.0)
        rad_at_300_num.set_color("#5BA8FF")
        self.add(rad_at_300_num)
        rad_at_300_num.add_updater(lambda m: m.set_value(rayleigh_jeans(300, T_SUN) / planck_spectral_radiance(500, T_SUN)))
        self.play(
            Create(rj_curve),
            FadeIn(marker_300_rj),
            run_time=0.35,
        )
        # Live value at 300 nm explodes
        self.play(
            rad_at_300_num.animate.set_value(min(8.0, rayleigh_jeans(300, T_SUN) / planck_spectral_radiance(500, T_SUN))),
            run_time=2.35,
            rate_func=linear,
        )
        self.wait(0.0)
        ...
        # ====================================================================
        # UV catastrophe wall + shock flash + label
        ...
        # ====================================================================
        # Staircase of energy steps beneath gold curve
        self.play(
        ...
        self.wait(3.35)
        self.play(run_time=0.3)

        # ====================================================================
        # BEAT A3.B2 | 15.00-18.00 s
        # ====================================================================
        # Blue curve crumbles into dashed fragments at the wall
        # Build dashed fragments: small DashedLine segments near the wall (200-320 nm)
        frag_segs = VGroup()
        for i in range(6):
            x0 = 205 + i * 12
            x1 = x0 + 6
            ...
            frag_segs.add(seg)
        self.play(
            Transform(rj_curve, VGroup(*frag_segs)),
            rad_at_300_num.animate.set_value(planck_spectral_radiance(300, T_SUN) / planck_spectral_radiance(500, T_SUN)),
            run_time=0.35,
        )
        rad_at_300_num.set_color("#FFD580")
        rad_at_300_num.add_updater(lambda m: m.set_value(planck_spectral_radiance(300, T_SUN) / planck_spectral_radiance(500, T_SUN)))
        self.wait(2.35)
        self.play(run_time=0.3)
        ...
        # ====================================================================
        # Hero composition: filament on left + calm gold curve + shattered blue ghost + equation
        # Make sure filament is glowing and equation is annotated
        self.play(
            Write(hero_eq),
            run_time=0.35,
        )
        # Let the composition breathe in a hold
        self.wait(3.35)
        self.play(run_time=0.3)
        ...
        # ====================================================================
        # Wien-displacement marker slides down to peak
        ...
        self.play(
            wien_label.animate.next_to(wien_marker, UP, buff=0.18).shift(RIGHT * 0.2),
            FadeIn(wien_label),
            FadeIn(wien_sub),
            run_time=2.35,
        )
        ...
        # ====================================================================
        # Filament dims, curves fade to single gold rainbow; three mini-panels
        self.play(
            temp_tracker.animate.set_value(3000.0),
            FadeOut(filament_glow),
            FadeOut(rad_label),
            FadeOut(rad_num),
            FadeOut(wall),
            FadeOut(wall_label),
            ...
            FadeOut(temp_num),
            FadeOut(temp_unit),
            FadeOut(rad_at_300_num),
            FadeOut(filament),
            FadeOut(marker_300_p),
            run_time=0.35,
        )
        # Single elegant gold rainbow over wavelength axis
        rainbow_axes = Axes(
        ...
        rainbow_lam = np.linspace(380, 1100, 180)
        ...
        # ====================================================================
        # Equation re-types under rainbow as final card; idle pulse on gold curve
        self.play(
        ...
        rainbow_curve.generate_target()
        rainbow_curve.target.move_to(np.array([0, 0.8, 0.0]))
        rainbow_curve.target.scale(1.2)
        self.play(
            MoveToTarget(rainbow_curve),
            run_time=0.0,
        )
        self.play(Write(closing_eq), run_time=1.35)

        # Idle loop: gentle pulse on gold curve + soft filament-like pulse
        pulse_tracker = ValueTracker(0.0)
        pulse_curve = always_redraw(lambda: rainbow_curve.copy().set_stroke(
            opacity=0.85 + 0.15 * math.sin(pulse_tracker.get_value())
        ).scale(1.0 + 0.02 * math.sin(pulse_tracker.get_value())))
        self.add(pulse_curve)
        self.play(
            pulse_tracker.animate.set_value(2.0 * math.pi),
            run_time=0.0,
        )
        self.wait(0.3)