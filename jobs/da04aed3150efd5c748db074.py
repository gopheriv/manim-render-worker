...
class AetherLabScene(Scene):
    def construct(self):
        ...
        x_label = Text("Wavelength ? (?m)", color=muted, font_size=26)
        ...
        uv_label = Text("Rayleigh?Jeans diverges as ? ? 0", color=cyan, font_size=25)
        ...
        self.play(Create(planck_curve), run_time=1.8)
        self.wait(1.2)
        ...
        peak_label = Text("?max = 0.97 ?m", color=gold, font_size=25)
        peak_label.next_to(peak_dot, UP, buff=0.18)
        ...
        peak_label_4500 = Text("?max = 0.64 ?m", color=gold, font_size=25)
        peak_label_4500.next_to(axes.c2p(peak_4500, 1.0), UP, buff=0.18)
        ...
        self.play(FadeIn(hero, shift=DOWN * 0.1), peak_dot.animate.scale(1.22), run_time=0.5)
        ...
        self.play(
            FadeOut(equation), FadeOut(hero), FadeOut(uv_label), FadeOut(uv_arrow),
        ...
        temp_legend = VGroup(
            VGroup(Line(LEFT * 0.25, RIGHT * 0.25, color=amber, stroke_width=4),
                   Text("4500 K ? ?max 0.64 ?m", color=amber, font_size=22)).arrange(RIGHT, buff=0.14),
            VGroup(Line(LEFT * 0.25, RIGHT * 0.25, color=coral, stroke_width=4),
                   Text("6000 K ? ?max 0.48 ?m", color=coral, font_size=22)).arrange(RIGHT, buff=0.14),
